...
- Nothing registered is known broken. The github server exposes six tools at
  v2.2.0; `get_github_label` and the `labels` filter on `list_github_issues`
  are the most recent additions. See `docs/07-plans/current.md`.

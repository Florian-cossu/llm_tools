...
| Instructions empty | `GITHUB_DEFAULT_USERNAME` unset, no `github_profiles` row is active, or server not restarted |
| Defaults absent from instructions | `stringOrNull` returned `null` — check for an empty `GITHUB_DEFAULT_USERNAME=`, or that a profile is actually toggled active in the control panel |
